...
def _collect_control_metrics(
    ...
    metrics = {
        ...
        "active_piece_drop": active_before - active_after,
    }
    return metrics
...
def tag_position(
    ...
    analysis_meta.setdefault("control_dynamics", {}).update(control_metrics)
    ...
    if allow_positional:
        control_gate_log = control_meta.setdefault("gates", {})

        tension_threshold = _control_tension_threshold(control_metrics["phase_bucket"])
        slowdown_gate = {
            "has_dynamic": has_dynamic_in_band,
            "played_kind": played_kind,
            "eval_drop_cp": delta_eval_cp,
            "eval_threshold_cp": CONTROL_EVAL_DROP,
            "volatility_drop_cp": control_metrics["volatility_drop_cp"],
            "volatility_threshold_cp": CONTROL_VOLATILITY_DROP_CP,
            "tension_delta": control_metrics["tension_delta"],
            "tension_threshold": tension_threshold,
            "opp_mobility_drop": control_metrics["opp_mobility_drop"],
            "mobility_threshold": CONTROL_OPP_MOBILITY_DROP,
        }
        slowdown_pass = (
            has_dynamic_in_band
            and played_kind == "positional"
            and delta_eval_cp <= CONTROL_EVAL_DROP
            and control_metrics["volatility_drop_cp"] >= CONTROL_VOLATILITY_DROP_CP
            and control_metrics["tension_delta"] <= tension_threshold
            and control_metrics["opp_mobility_drop"] >= CONTROL_OPP_MOBILITY_DROP
        )
        slowdown_gate["passed"] = slowdown_pass
        control_gate_log["slowdown"] = slowdown_gate
        if slowdown_pass:
            slowdown_metrics = {
                "eval_drop_cp": delta_eval_cp,
                "volatility_drop_cp": control_metrics["volatility_drop_cp"],
            ...
            control_candidates.append({"kind": "slowdown", "metrics": slowdown_metrics})

        captured_piece_type = control_metrics["captured_piece_type"]
        active_piece_drop = control_metrics["active_piece_drop"]
        simplify_gate = {
            "is_capture": control_metrics["is_capture"],
            "captured_piece_type": captured_piece_type,
            "active_piece_drop": active_piece_drop,
            "material_delta_self": material_delta_self,
            "volatility_drop_cp": control_metrics["volatility_drop_cp"],
        }
        capture_is_heavy = captured_piece_type in {chess.ROOK, chess.QUEEN}
        simplify_pass = (
            control_metrics["is_capture"]
            and (active_piece_drop >= CONTROL_SIMPLIFY_MIN_EXCHANGE or capture_is_heavy)
            and abs(material_delta_self) <= 0.5
            and control_metrics["volatility_drop_cp"] >= CONTROL_VOLATILITY_DROP_CP
        )
        simplify_gate["passed"] = simplify_pass
        control_gate_log["simplify"] = simplify_gate
        if simplify_pass:
            captured_label = chess.piece_name(captured_piece_type) if captured_piece_type else "trade"
            simplify_metrics = {
                "captured_piece": captured_label,
                "active_piece_drop": active_piece_drop,
                "volatility_drop_cp": control_metrics["volatility_drop_cp"],
            }
            control_candidates.append({"kind": "simplify", "metrics": simplify_metrics})

        freeze_gate = {
            "structure_gain": structure_gain,
            "opp_mobility_eval": control_metrics["opp_mobility_change_eval"],
            "tension_delta": control_metrics["tension_delta"],
            "tension_threshold": tension_threshold,
            "plan_drop_passed": plan_meta.get("passed"),
            "volatility_drop_cp": control_metrics["volatility_drop_cp"],
        }
        freeze_pass = (
            not plan_meta.get("passed")
            and structure_gain >= 0.2
            and control_metrics["opp_mobility_change_eval"] <= -0.2
            and control_metrics["tension_delta"] <= tension_threshold - 0.5
        )
        freeze_gate["passed"] = freeze_pass
        control_gate_log["freeze"] = freeze_gate
        if freeze_pass:
            freeze_metrics = {
                "structure_gain": structure_gain,
                "opp_mobility_drop": control_metrics["opp_mobility_drop"],
            ...
            control_candidates.append({"kind": "freeze", "metrics": freeze_metrics})

        king_safety_gate = {
            "king_safety_gain": control_metrics["king_safety_gain"],
            "opp_tactics_change": control_metrics["opp_tactics_change_eval"],
            "opp_mobility_drop": control_metrics["opp_mobility_drop"],
        }
        king_safety_pass = (
            control_metrics["king_safety_gain"] >= CONTROL_KING_SAFETY_THRESH
            and (
                control_metrics["opp_tactics_change_eval"] <= -0.1
                or control_metrics["opp_mobility_drop"] >= CONTROL_OPP_MOBILITY_DROP
            )
        )
        king_safety_gate["passed"] = king_safety_pass
        control_gate_log["king_safety"] = king_safety_gate
        if king_safety_pass:
            ks_metrics = {
                "king_safety_gain": control_metrics["king_safety_gain"],
                "opp_tactics_change_eval": control_metrics["opp_tactics_change_eval"],
            }
            control_candidates.append({"kind": "king_safety", "metrics": ks_metrics})
        ...
        if prophylactic_move:
            blunder_threat_drop = analysis_meta.get("blunder_threat_drop", 0.0)
            prophylaxis_gate = {
                "tactical_weight": tactical_weight,
                "tactical_threshold": CONTROL_TACTICAL_WEIGHT_CEILING,
                "mate_threat": analysis_meta.get("mate_threat", False),
                "blunder_threat_drop": blunder_threat_drop,
                "blunder_threshold": CONTROL_BLUNDER_THREAT_THRESH,
                "volatility_drop_cp": control_metrics["volatility_drop_cp"],
                "opp_mobility_drop": control_metrics["opp_mobility_drop"],
                "tension_delta": control_metrics["tension_delta"],
                "tension_threshold": tension_threshold,
            ...
            prophylaxis_pass = (
                tactical_weight <= CONTROL_TACTICAL_WEIGHT_CEILING
                and not analysis_meta.get("mate_threat", False)
                and blunder_threat_drop < CONTROL_BLUNDER_THREAT_THRESH
                and (
                    control_metrics["volatility_drop_cp"] >= CONTROL_VOLATILITY_DROP_CP
                    or control_metrics["opp_mobility_drop"] >= CONTROL_OPP_MOBILITY_DROP
                    or control_metrics["tension_delta"] <= tension_threshold
                )
            )
            prophylaxis_gate["passed"] = prophylaxis_pass
            control_gate_log["prophylaxis"] = prophylaxis_gate
            if prophylaxis_pass:
                pro_metrics = {
                    "volatility_drop_cp": control_metrics["volatility_drop_cp"],
                    "opp_mobility_drop": control_metrics["opp_mobility_drop"],
                }
                control_candidates.append({"kind": "prophylaxis", "metrics": pro_metrics})
    else:
        analysis_meta.setdefault("prophylaxis", {})
        plan_meta.setdefault("sampled", False)
        plan_meta.setdefault("passed", None)
    ...
    recent_info = control_meta.get("recent")
    ...
    if recent_info:
        last_kind = recent_info.get("kind")
        last_ply = recent_info.get("ply")
        if (
            last_kind
            and isinstance(last_ply, int)
            and isinstance(current_ply_index, int)
            and (current_ply_index - last_ply) <= CONTROL_COOLDOWN_PLIES
        ):
            control_candidates = [entry for entry in control_candidates if entry["kind"] != last_kind]

    control_meta["candidates"] = control_candidates
    if control_candidates:
        control_candidates.sort(key=lambda item: CONTROL_SUBTYPE_PRIORITY.get(item["kind"], 99))
        selected = control_candidates[0]
        summary_metrics = dict(selected["metrics"])
        summary_metrics.setdefault("volatility_drop_cp", control_metrics.get("volatility_drop_cp"))
        summary_metrics.setdefault("tension_delta", control_metrics.get("tension_delta"))
        summary_metrics.setdefault("opp_mobility_drop", control_metrics.get("opp_mobility_drop"))
        summary_metrics.setdefault("king_safety_gain", control_metrics.get("king_safety_gain"))
        summary_metrics.setdefault("opp_tactics_change_eval", control_metrics.get("opp_tactics_change_eval"))
        control_over_dynamics = True
        control_over_dynamics_subtype = selected["kind"]
        control_meta["subtype"] = control_over_dynamics_subtype
        control_meta["selected"] = {
            "kind": selected["kind"],
            "metrics": summary_metrics,
        }
        notes["control_over_dynamics"] = _format_control_summary(selected["kind"], summary_metrics)
        control_meta["recent"] = {
        ...
        control_meta.setdefault("selected", None)
    ...
    if (
        intent_label == "expansion"
        and delta_self_mobility > 0.3
        and drop_cp < 50
    ...
    if allow_positional or allow_structural:
        ...
        neutral_support = (
            abs(delta_self_mobility) >= TENSION_MOBILITY_NEAR
            or abs(delta_opp_mobility) >= TENSION_MOBILITY_NEAR
            or abs(contact_delta_played) >= TENSION_CONTACT_DELAY
        )
        ...
        if neutral_tension_creation and maneuver_opening:
            neutral_tension_creation = False
            notes.pop("neutral_tension_creation", None)
